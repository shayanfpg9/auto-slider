import { validate, sort, $, clearStatus, getTime } from "./functions.js";
...
fetch("./images.php")
  ...
  .then((imgs) => {
    ...
    if (haveLive(images)) {
      showSlide(haveLive(images, true , true));
    } else {
...
function inFullscreen(parent) {
  ...
  infoBtn.addEventListener("click", () => {
    ...
    ul.innerHTML = `
      <li>Event start:
       <span>
        ${getTime(new Date(imagesInfo[ActiveIndex].time.start))}
        </span>
      </li>

      <li>End of event: 
        <span>
          ${getTime(new Date(imagesInfo[ActiveIndex].time.expire))}
        </span>
      </li>
